// 1. 必須引入 Link 才能跳轉，必須引入 useAuthStore 才能判斷登入狀態
import useAuthStore from '@/features/auth/store/useAuthStore';
import { Link } from 'react-router-dom';

export default function Home() {
    // 2. 必須從 Store (大腦) 中取出 isLoggedIn
        ...
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="p-8 app-card rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="w-14 h-14 bg-blue-500/10 rounded-2xl flex items-center justify-center mb-6 text-3xl">📝</div>
                ...
                <div className="p-8 app-card rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="w-14 h-14 bg-green-500/10 rounded-2xl flex items-center justify-center mb-6 text-3xl">📊</div>
            ...
            <section className="mt-20 p-10 rounded-3xl bg-blue-600 text-white text-center shadow-2xl">
                ...
                {/* 3. 現在這裡就不會報錯了，因為上面有定義了 */}
                {isLoggedIn ? (
                    <Link to="/admin" className="inline-block bg-white text-blue-600 font-bold px-8 py-3 rounded-full hover:bg-opacity-90 transition-all">
                        進入管理後台
                    </Link>
                ) : (
                    <Link to="/login" className="inline-block bg-white text-blue-600 font-bold px-8 py-3 rounded-full hover:bg-opacity-90 transition-all">
                        免費開始使用
                    </Link>
                )}
            </section>
